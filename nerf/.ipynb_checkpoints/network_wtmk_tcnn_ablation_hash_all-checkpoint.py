...
from hash_encoding_wtmk_bit_all import HashEmbedder as HashEmbedder_msg
...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
                 ...
                 hash_select = 1, # 1 for finest; 0 for all hash; 2 for coarstest; 3 for middle hash.
                 **kwargs
                 ):
...
#         )
        ...
        if hash_select == 1:
            ...
            self.msg_encoder = HashEmbedder_msg(bounding_box=(0, 1), n_levels=16, n_features_per_level=2,\
                log2_hashmap_size=19, base_resolution=16, finest_resolution=2048, message_dim=message_dim)
        elif  hash_select == 2:
            self.msg_encoder = HashEmbedder_msg(bounding_box=(0, 1), n_levels=message_dim*2, n_features_per_level=2,\
                log2_hashmap_size=19, base_resolution=16, finest_resolution=16, message_dim=message_dim)
        elif  hash_select == 3:
            b = torch.exp((torch.log(torch.tensor(2048))-torch.log(torch.tensor(16)))/(16-1))
            resolution = torch.floor(16 * b**7)
            self.msg_encoder = HashEmbedder_msg(bounding_box=(0, 1), n_levels=message_dim*2, n_features_per_level=2,\
                log2_hashmap_size=19, base_resolution=resolution, finest_resolution=resolution, message_dim=message_dim)
...
#             x_feature = x_feature + msg_feature
            if self.hash_select == 1:
                x_feature[:, -2:] = x_feature[:, -2:] + msg_feature
            elif  self.hash_select == 0:
                x_feature = x_feature + msg_feature
            elif  self.hash_select == 2:
                x_feature[:, 0:2] = x_feature[:, 0:2] + msg_feature
            elif  self.hash_select == 3:
                x_feature[:, 14:16] = x_feature[:, 14:16] + msg_feature
#             x_feature[:, 0:2] = x_feature[:, 0:2] + msg_feature
