...
import raymarching
from .utils_wtmk_disen import custom_meshgrid
...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, message, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3), message = message)
    ...
    def run_cuda(self, rays_o, rays_d, message, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            sigmas, rgbs = self(xyzs, dirs, message)
    ...
    @torch.no_grad()
    def update_extra_state(self, message=None, decay=0.95, S=128):
                ...
                for ys in Y:
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            # query density
                            sigmas = self.density(cas_xyzs, message)['sigma'].reshape(-1).detach()
    ...
    def render(self, rays_o, rays_d, message=None, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    results_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], message, **kwargs)
            ...
            results = _run(rays_o, rays_d, message, **kwargs)
